...
const HealthCareAppointment = () => {
    ...
    return (
        <div className="relative flex flex-row shadow-xl overflow-x-auto whitespace-normal w-full h-[20rem] p-4 space-x-4">
            ...
            {
                healthcare.map((item) => (
                    <Link to={item.link} key={item.title} className="rounded-xl break-words cursor-pointer bg-gray-500 p-2 flex-shrink-0 xs:w-[50%] sm:w-[60%] md:w-[60%] lg:w-[40%] xl:w-[50%] h-full xs:p-4 hover:p-8">
