...
import TechnicalAndRepairDropDown from "../../AppointmentCustomDropDown/TechnicalAndRepairDropDown";
import InputFields from "./InputFields";
import { useSelector } from "react-redux";
import lang from "../../Utils/multiLanguageConfig";

const HomeServiceDom = ({handleHomeServiceForm, message, isError, isLoading, addressRef, genderRef, nextOfKinAddressRef, nextOfKinPhoneNumberRef, nextOfKinRef, appointmentDateRef, appointmentDescriptionRef, appointmentTimeRef, setMessage, handleSelected}) => {

    const langKey = useSelector((store) => store.config?.lang);

    return (
        <form onSubmit={handleHomeServiceForm} className="w-full space-y-4 mb-[2rem] mx-auto shadow-2xl bg-white flex flex-col p-4 rounded-2xl xs:w-[80%] sm:w-[80%] md:w-[80%] lg:w-[80%] xl:w-[50%]">
            <h1 className="break-words font-sans text-blue-800 font-bold  mb-8 animate-pulse xs:text-[0.8rem] sm:text-[1rem] md:text-[1.2rem] lg:text-[1.3rem] xl:text-[1.3rem]">{lang[langKey]?.appointmentsInputFields?.homeServiceHeading}</h1>
            {
                message && (
                    <div className={`break-words p-2 w-full font-sans xs:text-[0.6rem] sm:text-[0.8rem] md:text-[0.8rem] lg:text-[1rem] xl:text-[1rem] ${isError ? "text-red-600 bg-red-50" : "text-green-600 bg-green-50 font-semibold text-lg"}`}>
            ...
            type="text" 
            />
            <TechnicalAndRepairDropDown handleSelected={handleSelected}/>
            <InputFields
            autoComplete="on"
            id="next_of_kin"
