...
const FinancialAdvisoryDom = ({handleFinancialAdvisoryAppointmentForm, message, isError, addressRef, genderRef, nextOfKinRef, nextOfKinAddressRef, nextOfKinPhoneNumberRef, appointmentDateRef, appointmentDescriptionRef, appointmentTimeRef, isLoading, handleSelected, setMessage}) => {

    const langKey = useSelector((store) => store.config?.lang);

    return (
        <form onSubmit={handleFinancialAdvisoryAppointmentForm} className="w-full space-y-4 mb-[2rem] mx-auto shadow-2xl bg-white flex flex-col p-4 rounded-2xl xs:w-[80%] sm:w-[80%] md:w-[80%] lg:w-[80%] xl:w-[50%]">
            <h1 className="break-words font-sans text-blue-800 font-bold mb-8 animate-pulse xs:text-[0.8rem] sm:text-[1rem] md:text-[1.2rem] lg:text-[1.3rem] xl:text-[1.3rem]">{lang[langKey]?.appointmentsInputFields?.financialAdvisoryHeading}</h1>
            {
                message && (
                    <div className={`break-words p-2 w-full font-sans xs:text-[0.6rem] sm:text-[0.8rem] md:text-[0.8rem] lg:text-[1rem] xl:text-[1rem] ${isError ? "text-red-600 bg-red-50" : "text-green-600 bg-green-50"}`}>
                        <button type="button" className="text-[2rem] bg-white text-red-600 shadow-2xl w-8 h-8 text-center rounded-full border-[1px] border-red-300" onClick={() => setMessage(null)} title="cancel" aria-label="cancel">&times;</button>
...
export default FinancialAdvisoryDom;