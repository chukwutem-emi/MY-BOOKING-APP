...
const HealthCareCounselingSessionDom = ({handleCounselingSessionForm, message, isError, firstNameRef, lastNameRef, addressRef, emailAddressRef, genderRef, amountRef, userPhoneNumberRef, nextOfKinRef, nextOfKinAddressRef, nextOfKinPhoneNumberRef, appointmentDateRef, appointmentTimeRef, appointmentDescriptionRef, isLoading}) => {
    return (
        <form onSubmit={handleCounselingSessionForm} className="w-[50%] space-y-4 my-0 mx-auto shadow-2xl bg-white flex flex-col p-4 rounded-2xl xs:w-[90%] sm:w-[90%] md:w-[90%] lg:w-[90%] xl:w-[50%]">
            ...
            type="number"
            className="border-[1px] border-blue-600 outline-none text-[1.2rem] px-2 py-1 rounded-lg md:text-2xl lg:text-2xl xl:text-2xl" 
            placeholder="The amount to be paid is N15,000"
            ref={amountRef}
            id="amount"
            name="amount"
            autoComplete="on"
