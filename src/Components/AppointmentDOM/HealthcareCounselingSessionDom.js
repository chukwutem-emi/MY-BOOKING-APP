...
const HealthCareCounselingSessionDom = ({handleCounselingSessionForm, message, isError, addressRef, genderRef, nextOfKinRef, nextOfKinAddressRef, nextOfKinPhoneNumberRef, appointmentDateRef, appointmentTimeRef, appointmentDescriptionRef, isLoading, handleSelected, setMessage}) => {

    const langKey = useSelector((store) => store.config?.lang);

    return (
        <form onSubmit={handleCounselingSessionForm} className="w-full space-y-4 mb-[2rem] mx-auto shadow-2xl bg-white flex flex-col p-4 rounded-2xl xs:w-[80%] sm:w-[80%] md:w-[80%] lg:w-[80%] xl:w-[50%]">
            <h1 className="font-sans text-blue-800 font-bold mb-8 animate-pulse xs:text-[0.8rem] sm:text-[1rem] md:text-[1.2rem] lg:text-[1.3rem] xl:text-[1.3rem]">{lang[langKey]?.appointmentsInputFields?.healthcareCounselingSessionHeading}</h1>
            {
                message && (
...
export default HealthCareCounselingSessionDom;