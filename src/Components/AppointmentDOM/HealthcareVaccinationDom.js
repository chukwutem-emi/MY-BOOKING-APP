...
const HealthCareVaccinationDom = ({handleVaccinationAppointmentForm, message, isError, isLoading, firstNameRef, lastNameRef, addressRef, genderRef, emailAddressRef, userPhoneNumberRef, nextOfKinAddressRef, nextOfKinPhoneNumberRef, nextOfKinRef, amountRef, appointmentDateRef, appointmentDescriptionRef, appointmentTimeRef}) => {
    return (
        <form onSubmit={handleVaccinationAppointmentForm} className="w-[50%] space-y-4 my-0 mx-auto shadow-2xl bg-white flex flex-col p-4 rounded-2xl xs:w-[90%] sm:w-[90%] md:w-[90%] lg:w-[90%] xl:w-[50%]">
            <h1 className="text-center break-words font-sans text-blue-800 font-bold text-[1.5rem] mb-8 animate-pulse xs:text-lg sm:text-xl md:text-2xl lg:text-2xl xl:text-2xl">Healthcare Vaccination Appointment</h1>
            {
                message && (
            ...
            type="number"
            className="border-[1px] border-blue-600 outline-none text-[1.2rem] px-2 py-1 rounded-lg md:text-2xl lg:text-2xl xl:text-2xl" 
            placeholder="The amount to be paid is N50,000"
            ref={amountRef}
            id="amount"
            name="amount"
            autoComplete="on"
