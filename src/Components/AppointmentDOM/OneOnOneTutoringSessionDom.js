...
const OneOnOneTutoringSessionDom = ({handleOneOnOneTutorialForm, addressRef, appointmentDateRef, appointmentDescriptionRef, appointmentTimeRef, genderRef, isError, message, nextOfKinAddressRef, nextOfKinPhoneNumberRef, nextOfKinRef, isLoading, handleSelected, setMessage}) => {
    ...
    return (
        <form onSubmit={handleOneOnOneTutorialForm} className="w-full space-y-4 mb-[2rem] mx-auto shadow-2xl bg-white flex flex-col p-4 rounded-2xl xs:w-[80%] sm:w-[80%] md:w-[80%] lg:w-[80%] xl:w-[50%]">
            ...
            autoFocus
            />
            <button type="submit" className="bg-blue-800 p-2 rounded-lg break-words hover:bg-blue-600 outline-none text-white font-sans">
                {
                    isLoading ? (
