...
const ElectricalElectronicsRepairDom = ({handleElectricalElectronicsRepairForm, message, isError, isLoading, addressRef, genderRef, nextOfKinAddressRef, nextOfKinPhoneNumberRef, nextOfKinRef, appointmentDateRef, appointmentDescriptionRef, appointmentTimeRef, setMessage, handleSelected}) => {
    return (
        <form onSubmit={handleElectricalElectronicsRepairForm} className="w-[50%] space-y-4 mb-[2rem] mx-auto shadow-2xl bg-white flex flex-col p-4 rounded-2xl xs:w-[90%] sm:w-[90%] md:w-[90%] lg:w-[90%] xl:w-[50%]">
            <h1 className="break-words font-sans text-blue-800 font-bold text-[1.5rem] mb-8 animate-pulse xs:text-[1.2rem] sm:text-[1.3rem] md:text-[1.4rem] lg:text-[1.4rem] xl:text-[1.4rem]">Electrical/Electronics Repair Appointment</h1>
            {
                message && (
