   ...
   const logOut = () => {
    ...
    return (
        <div className="top-0 right-0 left-0 fixed shadow-lg z-40 flex  flex-row flex-nowrap justify-between p-4 xs:p-4 sm:p-8 md:p-8 lg:p-10 xl:p-12 bg-gray-600  md:flex-row md:mx-0 w-full">
            ...
            <button onClick={() => {isLogin === "Logout" ? setIsLogin("Login") : setIsLogin("Login")}} className="bg-blue-950 text-white font-sans font-extrabold h-10 p-2 mt-5 xs:text-[0.8rem] xl:text-2xl">{isLogin}</button>
            <NetworkStatus />
        </div>
    )
};
