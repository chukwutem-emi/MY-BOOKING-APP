...
import { BASE_URL, ONE_ON_ONE_TUTORING_URL } from "../../../Utils/constants";
...
const OneOnOneTutoring = () => {
    ...
    const handleOneOnOneTutorial = async (e) => {
        e.preventDefault();
        setIsLoading(true);
        const payload = {
        ...
        try {
            ...
            if (data.status === 401) {
                ...
                setMessage(`
                    ${json.one_one_tutoring}<br><a href=${json.googleCalenderEvent} target="_blank" rel="noopener noreferrer" title="click here" class="text-blue-500 justify-center items-center text-center font-semibold underline hover:text-lg hover:text-blue-800 my-0 mx-auto">Click here to view the one-on-one-tutoring appointment that you booked</a>
                    `);
                    setIsError(false);
            } else {
    ...
    return (
        <div className="mt-[16rem] overflow-x-hidden w-full items-center">
            <form onSubmit={handleOneOnOneTutorial} className="w-[50%] space-y-4 my-0 mx-auto shadow-2xl bg-white flex flex-col p-4 rounded-2xl xs:w-[90%] sm:w-[90%] md:w-[90%] lg:w-[90%] xl:w-[50%]">
